...
var AmqpServer = Service {
	AmqpUrl : "amqp://" + config.RabbitmqInc.User + ":" + config.RabbitmqInc.Password + "@" + config.RabbitmqInc.Host + ":" + config.RabbitmqInc.Port,
	ConnectionNum : 10,
	ChannelNum : 10,
}
...
func (S *Service) channelPool(){
	...
	S.ChannelConn = make(map[int]int)
